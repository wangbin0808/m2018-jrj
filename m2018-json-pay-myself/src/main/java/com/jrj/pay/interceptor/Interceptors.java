...
import com.jrj.pay.util.CookieUseridMd5;
...
public class Interceptors extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		log.info("--Interceptors请求的路径是"+request.getRequestURL());
		// 登录判断
/*		String login_state = CookiesUtil.getCookie(request, PayConstant.LOGIN_COOKIE_KEY, "");
		...
		if(CookieUseridMd5.loginState(request)){
			log.info("Interceptors--没有登录--");
			return false;
		};
		log.info("Interceptors--登录了--CookieUseridMd5==false");
		// 获取用户id
